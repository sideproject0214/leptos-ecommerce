...
impl EnvConfig for EnvValue {
  ...
  fn load_config(&mut self) {
    let rust_env = dotenv::var("RUST_ENV").unwrap_or_else(|_| "development".to_string());
    ...
    self.api_address = env::var("API_ADDRESS").expect("API_ADDRESS Not Exist");
    self.db_dialect = env::var("DB_DIALECT").expect("DB_DIALECT Not Exist");
    self.db_username = env::var("DB_USERNAME").expect("DB_USERNAME Not Exist");
    self.db_password = env::var("DB_PASSWORD").expect("DB_PASSWORD Not Exist");
    self.db_host = env::var("DB_HOST").expect("DB_HOST Not Exist");
    self.db_port = env::var("DB_PORT").expect("DB_PORT Not Exist");
    self.db_database = env::var("DB_DATABASE").expect("DB_DATABASE Not Exist");
  }
