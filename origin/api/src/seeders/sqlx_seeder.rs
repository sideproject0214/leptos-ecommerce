              ...
              if let Some(first_part) = file_name.split(".").next() {
                ...
                for json_value in json_data {
                  if let Some(field_value) = json_value.get(first_part) {
                    ...
                    // let field_value = arr_field_value.first().unwrap();
                    // println!("arr_field_value  {:?}", arr_field_value);
                    ...
                    for each in arr_field_value {
                      ...
                      // {} 형태로 만들어짐
                      // let json_data = serde_json::from_str(s)
                      ...
                      // 여기서 변수 형태로 query를 정하지 않으면 작동이 되지 않는다
                      // 일반적으로 Rust의 문자열 리터럴과 같은
                      // 정적인 문자열은 컴파일 타임에 이미 알려져 있기 때문에 동적으로
                      // 생성된 문자열을 사용하는 경우 sqlx::query 매크로에 직접 적는 것은
                      // 불가능합니다. sqlx::query 매크로는 컴파일 타임에 알려진 쿼리만을
                      // 처리할 수 있습니다.

                      // 따라서 sqlx::query 매크로에는 컴파일 타임에 알려진 정적인 쿼리만
                      // 사용할 수 있으며, 동적으로 생성된 쿼리를 사용하려면 String이나
                      // &str로 만들어진 변수를 사용해야 합니다. 이렇게 하면 컴파일 타임에
                      // 쿼리가 결정되고, 실행 시간에 동적으로 생성되는 쿼리를 처리할 수
                      // 있습니다.

                      // 정리하자면, sqlx::query 매크로에는 정적으로 알려진 쿼리만을
                      // 사용할 수 있으며, 동적으로 생성된 쿼리는 런타임에 결정되기 때문에
                      // 매크로에 직접 전달할 수 없습니다.
                      ...
                      // let json_data_string = serde_json::to_string(&each).unwrap();

                      // extract_field_names(&json_data, &mut field_names);
                      // extract_field_values(&json_data_string, &mut field_values);

                      // for name in &field_names {
                      //   println!("json field_name : {}", name,);
                      // }
                      // for value in &field_values {
                      //   println!("json field_value : {}", value,);
                      // }
